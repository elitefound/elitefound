<?php
$footer = '<footer class="seccion_oscura">
<div class="textoJustificado" style="width: 95%; margin: auto">
  <div class="row">
    <div class="col">
      <span style="font-size: 10px; line-height: 1.5; margin: 0;">El contenido aquí presentado tiene únicamente fines informativos a la fecha de su publicación y puede cambiar debido a eventos del mercado u otros factores. ELITE FOUND no considera tus objetivos ni tu situación financiera personal. Nada de lo expresado aquí constituye una recomendación de inversión específica. Eres responsable de evaluar si una inversión, estrategia o producto es adecuado para ti, considerando tus metas, situación financiera y los riesgos asociados. El rendimiento pasado no garantiza resultados futuros, y toda inversión conlleva potencial de pérdida.</span>
    </div>
  </div>
</div>

<div class="row">
    <div class="col-4">
        <p style="text-align: right;">
            <a style=" font-size: 10px;">
              <svg class="ps-1" id="Capa_1" data-name="Capa 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 571.13 607.86">
                  <path d="M560.72,491.08c-10.72,11.1-20.93,22.74-32.24,33.2-44.46,41.09-96.26,68.19-156.14,78.58-81.01,14.06-156.33-1.5-224.3-47.57C69.32,501.94,22.82,426.92,5.87,333.85-7.37,261.09,1.86,190.79,32.81,123.46c.69-1.49,1.44-2.96,2.88-4.21-1.07,3.97-2.22,7.91-3.19,11.91-29.63,122.32-4.07,231.48,80.73,324.94,51.37,56.61,116.23,90.62,192.17,101.66,92.73,13.49,176.33-9.28,250.83-65.84,1.02-.77,2.09-1.48,3.13-2.21,.45,.46,.9,.91,1.35,1.37Z"/>
                  <path d="M165.94,452.62c-74.76-56.13-114.74-130.47-112.78-224.53,1.92-91.79,43.65-163.24,117.79-216.33-93.67,145.96-100.35,292.68-5.01,440.86Z"/>
                  <path d="M173.36,224.69c12.66,95.44,92.09,155.45,173.17,157.64,87.91,2.37,170.1-61.34,183.3-156.71,2.48,29.54-2.46,57.88-14.5,85-12.12,27.29-29.99,50.19-53.59,68.52-61.07,47.44-144.85,50.71-209.33,8.1-65.38-43.21-85.07-115.69-79.05-162.55Z"/>
                  <path d="M512.44,363.25c29.66-40.97,41.13-86.35,32.36-136.21-6.79-38.63-25.29-71.19-54.34-97.58-59.77-54.32-153.78-61.78-223.66-7.55,3.75-10.19,36.37-35.29,59.25-45.63,23.43-10.58,48.01-15.69,73.69-15.39,72.97,.83,137.84,47.33,161.62,115.6,24.06,69.06,2.06,143.29-48.94,186.78Z"/>
                  <path d="M136.16,212.9c-.43-57.79,31.98-144.63,120.89-189.43,81.12-40.87,175.8-25.67,236.48,24.76-13.53-4.1-27.18-8.73-41.09-12.35-46.41-12.08-93-14.35-139.53-.64-48.03,14.15-86.53,42.49-118.48,80.34-23.47,27.8-41.8,58.75-55.87,92.25-.61,1.45-1.34,2.85-2.4,5.07Z"/>
                  <path d="M412.23,237.7c.25,30.38-24.49,55.9-54.5,56.2-31.83,.32-57.31-24.11-57.54-55.17-.24-31.26,24.35-56.38,55.52-56.7,31.14-.32,56.27,24.43,56.53,55.67Z"/>
              </svg>
              ELITE FOUND
            </a>
        </p>
    </div>
    <div class="col-4">
      <p style="text-align: center;">
        <a style="cursor: pointer; font-size: 10px;" data-bs-toggle="modal" data-bs-target="#modalGeneralTerms">
          Privacidad y legal
        </a>
      </p>
    </div>
    <div class="col-4">
      <p>
        <a style="text-align: left; font-size: 10px;" href="contacto.php">
          Contacto
        </a>
      </p>
    </div>
</div>
</footer>
<div class="modal" tabindex="-1" id="modalGeneralTerms">
<div class="modal-dialog modal-fullscreen">
<div class="modal-content seccion_oscura">
  <div class="modal-header">
    <h5 class="modal-title">GENERAL TERMS AND CONDITIONS</h5>
    <div data-bs-theme="dark">
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
    </div>
  </div>
  <div class="modal-body">
    <iframe class="pdfVisual" src="terms and conditions/GENERAL TERMS AND CONDITIONS/index.html">
    </iframe>
  </div>
  <div class="modal-footer">
    <button type="button" class="btn btn-light" data-bs-toggle="modal" data-bs-target="#modalGeneralTerms">GENERAL TERMS AND CONDITIONS</button>
    <button type="button" class="btn btn-light" data-bs-toggle="modal" data-bs-target="#modalsecurity">SECURITY AND PRIVACY</button>
    <button type="button" class="btn btn-light" data-bs-toggle="modal" data-bs-target="#modalspecific">SPECIFIC TERMS OF THE INVESTMENT FUND</button>
  </div>
</div>
</div>
</div>                  
<div class="modal" tabindex="-1" id="modalsecurity">
<div class="modal-dialog modal-fullscreen">
<div class="modal-content seccion_oscura">
  <div class="modal-header">
    <h5 class="modal-title">SECURITY AND PRIVACY</h5>
    <div data-bs-theme="dark">
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
    </div>
  </div>
  <div class="modal-body">
    <iframe class="pdfVisual" src="terms and conditions/SECURITY AND PRIVACY/index.html">
    </iframe>
  </div>
  <div class="modal-footer">
    <button type="button" class="btn btn-light" data-bs-toggle="modal" data-bs-target="#modalGeneralTerms">GENERAL TERMS AND CONDITIONS</button>
    <button type="button" class="btn btn-light" data-bs-toggle="modal" data-bs-target="#modalsecurity">SECURITY AND PRIVACY</button>
    <button type="button" class="btn btn-light" data-bs-toggle="modal" data-bs-target="#modalspecific">SPECIFIC TERMS OF THE INVESTMENT FUND</button>
  </div>
</div>
</div>
</div>                  
<div class="modal" tabindex="-1" id="modalspecific">
<div class="modal-dialog modal-fullscreen">
<div class="modal-content seccion_oscura">
  <div class="modal-header">
    <h5 class="modal-title">SECURITY AND PRIVACY</h5>
    <div data-bs-theme="dark">
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
    </div>
  </div>
  <div class="modal-body">
    <iframe class="pdfVisual" src="terms and conditions/SPECIFIC TERMS OF THE INVESTMENT FUND/index.html">
    </iframe>
  </div>
  <div class="modal-footer">
    <button type="button" class="btn btn-light" data-bs-toggle="modal" data-bs-target="#modalGeneralTerms">GENERAL TERMS AND CONDITIONS</button>
    <button type="button" class="btn btn-light" data-bs-toggle="modal" data-bs-target="#modalsecurity">SECURITY AND PRIVACY</button>
    <button type="button" class="btn btn-light" data-bs-toggle="modal" data-bs-target="#modalspecific">SPECIFIC TERMS OF THE INVESTMENT FUND</button>
  </div>
</div>
</div>
</div>';